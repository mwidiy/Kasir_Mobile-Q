...
object RetrofitClient {
    ...
    private val authInterceptor = Interceptor { chain ->
        ...
        id.quacxel.mejapesan.utils.SessionManager.jwtToken?.let { token ->
    ...
    private val retryInterceptor = Interceptor { chain ->
        ...
        while (!responseOK && tryCount < maxLimit) {
            try {
                ...
                if (!responseOK) {
                    // It's a 500 Internal Server Error (or similar)
                    // We shouldn't necessarily retry 500s infinitely, but if we do, we MUST close it!
                    break // Do NOT retry 500s. Just let it return the 500 response so we see the real error!
                }
            } catch (e: Exception) {
                // If it's a SocketTimeoutException or network drop, retry as well
                response?.close() // Ensure we close any half-open response before retrying
                tryCount++
